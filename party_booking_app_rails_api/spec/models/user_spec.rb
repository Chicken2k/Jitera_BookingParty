require 'rails_helper'

RSpec.describe User, type: :model do
  let(:user) do
    build(:user)
  end

  describe 'Assocations' do
    it { is_expected.to have_many(:partybookings) }
  end

  describe 'Valid subject' do
    it 'valid user' do
      expect(user).to be_valid
    end
  end

  describe 'Format option validations' do
    it { expect(user.email).to match(URI::MailTo::EMAIL_REGEXP) }
    it { expect(Faker::Lorem.sentence).not_to match(URI::MailTo::EMAIL_REGEXP) }
  end

  describe 'Invalid length validations' do
    it 'validates name max length' do
      user.name = 'a' * 256
      expect(user).not_to be_valid
    end

    it 'validates username max length' do
      user.username = 'a' * 256
      expect(user).not_to be_valid
    end

    it 'validates password max length' do
      user.password = 'a' * 65_536
      expect(user).not_to be_valid
    end

    it 'validates email max length' do
      user.email = 'a' * 256
      expect(user).not_to be_valid
    end
  end

  describe 'Invalid boolean validations' do
    it { expect(user.isactive).to be_in([true, false]) }

    it 'does not a valid boolean' do
      user = described_class.new
      user.isactive = nil

      expect(user.isactive).not_to be_in([true, false])
    end
  end

  describe 'Validate uniqueness' do
    subject { build(:user) }

    it { is_expected.to validate_uniqueness_of(:email) }
  end
end
